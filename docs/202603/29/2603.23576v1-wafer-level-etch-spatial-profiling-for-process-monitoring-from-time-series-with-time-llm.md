...
## 摘要
从原位工艺信号中理解晶圆级空间变化对于先进等离子体刻蚀工艺监控至关重要。虽然大多数数据驱动方法侧重于平均刻蚀速率等标量指标，但实际工艺质量是由晶圆上复杂的二维空间分布决定的。本文提出了一种空间回归模型，直接从多通道原位工艺时间序列预测晶圆级刻蚀深度分布。我们提出了一种基于 Time-LLM 的空间回归模型，通过重新设计输入嵌入和输出投影，将大语言模型（LLM）重编程从传统的时间序列预测扩展到晶圆级空间估计。利用 BOSCH 等离子体刻蚀数据集，我们证明了在数据受限条件下的稳定性能，支持了基于 LLM 重编程进行晶圆级空间监控的可行性。
