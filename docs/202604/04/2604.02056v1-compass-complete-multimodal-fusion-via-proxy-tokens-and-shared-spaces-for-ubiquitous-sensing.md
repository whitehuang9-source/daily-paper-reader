...
tldr: 针对多模态感知中模态缺失导致融合结构不一致的问题，本文提出COMPASS框架。该框架基于“融合完整性”原则，通过在共享潜空间中利用观测模态生成缺失模态的代理Token，确保融合头始终接收固定数量的输入。通过代理对齐、共享空间正则化和判别性监督，COMPASS在多个数据集上显著提升了缺失模态场景下的感知性能，为鲁棒的多模态融合提供了新思路。
selection_source: fresh_fetch
motivation: 现有方法在模态缺失时通常会改变融合头的输入结构，导致训练与推理不一致并削弱跨模态交互能力。
method: 在共享潜空间中利用成对源到目标生成器合成缺失模态的代理Token，并结合对齐损失和判别性监督确保其任务相关性。
result: 在XRF55、MM-Fi和OctoNet等数据集的多种单模态及多模态缺失场景下，COMPASS的性能均优于现有先进方法。
conclusion: 维持模态完整的融合接口是提升多模态感知系统鲁棒性的一种简单且极其有效的设计原则。
---
